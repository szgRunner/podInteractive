...
func Resize(req *restful.Request, resp *restful.Response) {
	...
	t, ok := t1.getTerminal()
	if !ok {
		_ = resp.WriteErrorString(500, t1.key()+"没有 Exec 实例")
		return
	}
	size := &struct {
		Width  uint16
	...
	if err != nil {
		_ = resp.WriteErrorString(500, err.Error())
		return
	}
...
func (t *terminal) executor(ctx context.Context, eg *errgroup.Group) {
	...
	eg.Go(func() error {
		...
		if err != nil {
			_ = fmt.Errorf("clientcmd.BuildConfigFromFlags =%s ", err.Error())
			return err
		}
		exec, err := remotecommand.NewSPDYExecutor(restConfig, http.MethodPost, req.URL())
		if err != nil {
			_ = fmt.Errorf("remotecommand.NewSPDYExecutor =%s ", err.Error())
			return err
		}
